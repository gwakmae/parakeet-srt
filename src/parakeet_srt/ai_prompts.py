...
1.  **자동 목차 생성 (매우 중요):** 문서의 가장 첫 부분(제목 및 간단한 요약 바로 아래)에 옵시디언 'Dynamic Table of Contents' 플러그인이 인식할 수 있도록 아래 예시와 같이 위아래로 줄바꿈이 포함된 `toc` 코드 블록을 정확히 삽입해 주세요. 절대로 수동으로 목차 목록을 만들지 마세요.
...
최종 결과물은 제가 옵시디언에 바로 복사해서 붙여넣을 수 있도록, 완성된 마크다운 전체를 단 하나의 마크다운 코드 블록 안에 담아서 제공해 주세요.
